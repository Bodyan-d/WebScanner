...
export default function ScanForm({ onStart, onDone, onError }) {
  ...
  async function submit(e) {
    ...
    const sqlmap_args = [`--level=${level}`, `--risk=${risk}`, `--threads=${threads}`, "--random-agent", "--batch"];
    ...
    try {
      ...
      if (!resBase.ok) throw new Error(`Server ${resBase.status}: ${await resBase.text()}`);
      const jsonBase = await resBase.json();

      // Передаємо результат базового скану
      onDone && onDone(jsonBase);

      if (!runSqlmap) return;

      // 2️⃣ Запускаємо sqlmap
      const scanId = jsonBase.scan_id;
      if (!scanId) throw new Error("scan_id not returned by server — cannot run sqlmap.");

      onDone && onDone(jsonBase, true); // включаємо loading.sqlmap

      const sqlmapPayload = { url, scan_id: scanId, run_sqlmap: true, sqlmap_args };
      ...
      if (!resSqlmap.ok) throw new Error(`SQLMap run failed ${resSqlmap.status}: ${await resSqlmap.text()}`);
      const jsonSqlmap = await resSqlmap.json();

      onDone && onDone(jsonSqlmap, true); // передаємо результат sqlmap
    } catch (err) {
      onError && onError(err);
    }
  }
