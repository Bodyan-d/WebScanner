  ...
  return (
    <div className="findings-grid" style={{ display: "grid", gap: 10 }}>
      {items.map((it, idx) => {
        ...
        const color = vulnerable ? "#ef4444" : "#22c55e"; 
        ...
        return (
          ...
          >
            ...
            <div className="" style={{ marginTop: 6 }}>
              ...
              {it.param && (
                <div>
                  <strong>Parameter:</strong> <code style={{color: "#000"}}>{it.param}</code>
                </div>
              )}
              {it.payload && (
                <div>
              ...
              {it.marker && (
                <div>
                  <strong>Marker:</strong> <code style={{color: "#000"}}>{it.marker}</code>
                </div>
              )}
              {it.similarity && (
                <div>
...
function SqlmapView({ sqlmap }) {
  const results = Array.isArray(sqlmap?.raw) ? sqlmap.raw : [];

  if (!results.length) {
    return <div className="secure">✅ Secure — No sqlmap vulnerabilities found</div>;
  }


  const grouped = results.reduce((acc, r) => {
    const key = `${r.url}::${r.message}`;
    if (!acc[key]) acc[key] = { ...r, occurrences: 0 };
  ...
  return (
    <div>
      {Object.values(grouped).map((r, i) => {
        ...
        return (
          <div key={i} style={{ borderLeft: `4px solid ${color}`, padding: 8, marginBottom: 10 }}>
            <div style={{ color, fontWeight: 600 }}>
              {r.level?.toUpperCase()}: {r.message} ({r.occurrences} times)
            </div>
            <div style={{ fontSize: 12, color: "#aaa" }}>{r.url}</div>
          </div>
        );
      })}
    </div>
